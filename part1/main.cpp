...
void TestRanking() {
  const vector<string> docs = {
    ...
    "moscow is the capital of russia", // 7 -- moskow - 1; is - 1; the - 1; capital - 1; of - 1; russia - 1; =>hitcount = 1 + 1+ 1 + 1 + 1 + 1 = 6;
    ...
    "welcome to moscow the capital of russia the third rome", // 14 -- moskow - 1; is - 0; the - 2; capital - 1; of - 1; russia - 1; =>hitcount = 1 + 0 + 2 + 1 + 1 + 1 = 6;
...
int main() {
	...
	istringstream docs_input(Join('\n', docs));
	istringstream queries_input(Join('\n', queries));
	SearchServer sr(docs_input);
	sr.AddQueriesStream(queries_input, cout);
}
